...
contract LotteryToken is ERC721Enumerable, Ownable {
  ...
  function getShip(uint256 tokenId)
    public
    view
    returns (SpaceShips.Ship memory)
  {
  ...
  function contractURI() public view returns (string memory) {
    return
      string(abi.encodePacked("ipfs://", CID, "/collection-metadata.json"));
  }
   ...
   */
  function mint(address recipient, SpaceShips.Ship memory spaceShip)
    public
    onlyOwner
    returns (uint256)
  {
